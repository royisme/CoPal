"""Planning stage command implementation."""
...
def plan_command(target: str) -> int:
    ...
    role_template = templates_root / "knowledge-base" / "roles" / "planner.md"
    ...
    try:
        ...
        print(f"\n✓ Planning stage prompt generated: {prompt_path}\n")
        print("Please have Codex perform the following:")
        print(f"  1. Read the prompt file: {prompt_path}")
        print(f"  2. After completing plan, save artifacts to: {artifacts_dir / 'plan.md'}\n")
        print("Next step:")
        print("  copal implement\n")

        maybe_record_stage_memory(
            target_root=target_root,
            memory_type=MemoryType.PLAN,
            content=f"Planning stage prompt generated at {prompt_path}",
            metadata={
                "stage": "plan",
